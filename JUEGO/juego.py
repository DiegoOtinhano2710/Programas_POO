...
class aplicacion(tk.Tk):
    ...
    def __init__(self):
        ...
        self.__marcador.grid(row=0, column=0, columnspan=2, sticky='we', ipady=50, ipadx=50, pady=10, padx=10)
        fuente = ('Arial', 20)    
        #esto es para poner el texto "Puntaje"
        ...
        if self.__boton_iluminado is None:  
            if boton_elegido in self.__botones:                 #si el boton que clickeo es uno de los 4 botones
                ...
                if self.__lista_botones_usuario == self.__lista_botones[:len(self.__lista_botones_usuario)]:
                    ...
                    self.guardar_json()
                    messagebox.showinfo("GAME OVER", f"Puntaje obtenido: {self.__contador_puntos.get()}")
                    self.destroy()
